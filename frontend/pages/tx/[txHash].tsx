...
function TransactionDetailPage() {
    ...
    async function fetchTransactionDetails() {
      ...
      try {
        ...
        const operations = await server
          .operations()
          .forTransaction(txHash as string)
          .call();
        ...
        const paymentOp = operations.records.find((op) => op.type === "payment") as
          PaymentOp | undefined;
        if (paymentOp) {
        ...
        const detail: TransactionDetail = {
          ...
          fee: (parseInt(String(tx.fee_charged || tx.max_fee), 10) / 10000000).toFixed(7),
  ...
  const getTimelineSteps = (): TimelineStep[] => {
    ...
    const steps: TimelineStep[] = [
      ...
      {
        label: transaction.successful ? "Confirmed" : "Failed",
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-cosmos-950 dark:to-cosmos-900 pb-20 md:pb-8">
        <div className="container mx-auto px-4 py-6 max-w-4xl">
          <div className="card text-center py-12">
            ...
            <button onClick={() => router.back()} className="btn-secondary text-sm py-2 px-4">
              Go Back
            </button>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-cosmos-950 dark:to-cosmos-900 pb-20 md:pb-8">
      <div className="container mx-auto px-4 py-6 max-w-4xl">
        <div className="mb-6">
          <button
            onClick={() => router.back()}
            className="text-sm text-stellar-600 dark:text-stellar-400 hover:underline mb-3 inline-flex items-center gap-1"
          >
            ...
            <div className="card">
              ...
              {transaction.amount && transaction.asset && (
                <div className="mb-6 p-4 rounded-xl bg-slate-50 dark:bg-white/5 text-center">
                  <div className="flex items-center justify-center gap-3 mb-2">
                    ...
                    <h3 className="text-sm font-medium text-slate-600 dark:text-slate-400 capitalize">
                      {transaction.type || "Payment"}
                    </h3>
                  </div>
                  <p className="text-3xl font-bold text-slate-900 dark:text-white">
                    {formatAsset(transaction.amount, transaction.asset)}
                  </p>
                </div>
              )}
              <div className="space-y-3 text-sm">
                <div className="flex justify-between items-start">
                  ...
                  >
                    {transaction.successful ? "Confirmed" : "Failed"}
                  </span>
                </div>
                <div className="flex justify-between items-start">
              ...
              <div className="card">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <span className="font-mono text-sm text-slate-900 dark:text-slate-200 break-all">
                      {transaction.from}
                    </span>
                  </div>
                  <div>
            ...
            <div className="card">
              ...
              <div className="space-y-4 text-sm">
                <div>
                  ...
                  <p className="font-mono text-xs text-slate-900 dark:text-slate-200 break-all bg-slate-50 dark:bg-white/5 p-2 rounded">
                    {transaction.hash}
                  </p>
                </div>
                <div className="flex justify-between items-start">
                  <span className="text-slate-600 dark:text-slate-400">Ledger</span>
                  <div className="flex items-center gap-2">
                    <LedgerIcon className="w-4 h-4 text-slate-400" />
                ...
                <div className="flex justify-between items-start">
                  <span className="text-slate-600 dark:text-slate-400">Fee Charged</span>
                  <span className="font-mono font-medium text-slate-900 dark:text-white">
                    {transaction.fee} XLM
                  </span>
                </div>
                ...
                {transaction.memo && (
                  <div>
                    ...
                    <div className="bg-slate-50 dark:bg-white/5 p-3 rounded-lg">
                      <p
                        className={clsx(
                          "text-slate-900 dark:text-slate-200 break-words",
                          !showFullMemo && transaction.memo.length > 100 && "line-clamp-2",
                        )}
                      >
                        {transaction.memo}
                      </p>
                      {transaction.memo.length > 100 && (
                        <button
                          onClick={() => setShowFullMemo(!showFullMemo)}
                          className="text-xs text-stellar-600 dark:text-stellar-400 hover:underline mt-1"
                        >
                          {showFullMemo ? "Show less" : "Show more"}
                        </button>
                      )}
                    </div>
                  </div>
