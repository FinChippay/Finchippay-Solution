...
function sanitizePublicKey(req, res, next) {
  ...
  if (sanitized.length !== 56 || !sanitized.startsWith("G")) {
    return sendError(res, "VAL_INVALID_PUBLIC_KEY", {
      details: { reason: "Must be 56 characters starting with 'G'." },
    });
  }
  ...
  if (!/^G[A-Z2-7]{55}$/.test(sanitized)) {
    return sendError(res, "VAL_INVALID_PUBLIC_KEY", {
      details: { reason: "Contains characters outside the base-32 alphabet (A-Z, 2-7)." },
    });
  }
...
function sanitizeUsername(req, res, next) {
  ...
  if (!sanitized || sanitized.length < 3 || sanitized.length > 20) {
    return sendError(res, "VAL_INVALID_USERNAME");
  }
