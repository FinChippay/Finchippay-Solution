...
describe("Scheduled Transactions Routes", () => {
  ...
  describe("POST /api/scheduled-transactions", () => {
    it("returns 201 with scheduled transaction on success", async () => {
      ...
      const res = await request(app).post("/api/scheduled-transactions").send(mockTx);

      expect(res.status).toBe(201);
      expect(res.body.id).toBe("tx-1");
      expect(res.body.ownerPk).toBe("GABC123");
      expect(scheduledTransactionService.createSchedule).toHaveBeenCalledWith(mockTx);
    });
  ...
  describe("POST /api/scheduled-transactions/pending/:id/submit", () => {
    it("returns 200 with result on success", async () => {
      ...
      expect(scheduledTransactionService.submitPendingExecution).toHaveBeenCalledWith(
        "execution-1",
        "AAAAAgAAAAC...",
      );
    });
  ...
  describe("GET /api/scheduled-transactions/:publicKey", () => {
    it("returns schedules for a given public key", async () => {
      ...
      expect(scheduledTransactionService.listSchedules).toHaveBeenCalledWith("GABC123");
    });
  });

  describe("GET /api/scheduled-transactions/:publicKey/pending", () => {
    it("returns pending executions for a given public key", async () => {
      const mockPending = [{ id: "execution-1", ownerPk: "GABC123", status: "awaiting_signature" }];

      scheduledTransactionService.listPendingExecutions.mockReturnValue(mockPending);

      const res = await request(app).get("/api/scheduled-transactions/GABC123/pending");

      expect(res.status).toBe(200);
      expect(res.body).toHaveLength(1);
      expect(res.body[0].id).toBe("execution-1");
      expect(scheduledTransactionService.listPendingExecutions).toHaveBeenCalledWith("GABC123");
    });
  });
  ...
  describe("DELETE /api/scheduled-transactions/:id", () => {
    ...
    it("returns 404 when schedule is not found", async () => {
      ...
      const res = await request(app).delete("/api/scheduled-transactions/tx-999");
