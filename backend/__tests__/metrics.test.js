...
function resetCounters() {
  ...
  metrics.register.getSingleMetric("db_query_duration_seconds")?.reset();
  metrics.register.getSingleMetric("horizon_request_duration_seconds")?.reset();
  metrics.register.getSingleMetric("horizon_requests_total")?.reset();
  metrics.register.getSingleMetric("webhook_deliveries_total")?.reset();
  metrics.register.getSingleMetric("contract_events_indexed_total")?.reset();
  metrics.register.getSingleMetric("active_users")?.reset();
  metrics.register.getSingleMetric("payments_volume_total")?.reset();
  metrics.register.getSingleMetric("error_count_total")?.reset();
}
...
describe("metricsService — http_requests_total", () => {
  it("increments after inc()", async () => {
    ...
    await expect(
      getMetricValue("http_requests_total", {
        method: "GET",
        route: "/api/health",
        status_code: "200",
      }),
...
describe("metricsService — http_request_duration_seconds", () => {
  it("records duration in histogram", async () => {
    ...
    const snapshot = await metrics.register.getSingleMetric("http_request_duration_seconds").get();
...
describe("metricsService — http_requests_in_flight", () => {
  it("inc and dec modify gauge", async () => {
    ...
    metrics.httpRequestsInFlight.inc({ method: "GET", route: "/api/health" });
    await expect(
      getMetricValue("http_requests_in_flight", { method: "GET", route: "/api/health" }),
    ).resolves.toBe(2);
...
describe("metricsService — horizon metrics", () => {
  it("horizon_requests_total increments", async () => {
    ...
    await expect(
      getMetricValue("horizon_requests_total", {
        operation: "account",
        status: "success",
      }),
    ).resolves.toBe(1);
  ...
  it("horizon_request_duration_seconds records duration", async () => {
    ...
    const snapshot = await metrics.register
      .getSingleMetric("horizon_request_duration_seconds")
      .get();
    expect(snapshot.values.some((v) => v.metricName?.endsWith("_count") && v.value === 1)).toBe(
      true,
    );
...
describe("metricsService — webhook deliveries", () => {
  it("increments webhook_deliveries_total by status", async () => {
    ...
    await expect(getMetricValue("webhook_deliveries_total", { status: "success" })).resolves.toBe(
      1,
    );
    await expect(getMetricValue("webhook_deliveries_total", { status: "failed" })).resolves.toBe(1);
  });
});
...
describe("metricsService — contract events", () => {
  it("increments contract_events_indexed_total by event_type", async () => {
    ...
    await expect(
      getMetricValue("contract_events_indexed_total", { event_type: "payment" }),
    ).resolves.toBe(1);
    await expect(
      getMetricValue("contract_events_indexed_total", { event_type: "trustline" }),
    ).resolves.toBe(1);
  });
});

describe("metricsService — active_users gauge", () => {
  it("set and read gauge value", async () => {
    metrics.activeUsers.set(42);
    const snap = await metrics.register.getSingleMetric("active_users").get();
...
describe("metricsService — payments_volume_total", () => {
  it("increments by asset", async () => {
    ...
    await expect(getMetricValue("payments_volume_total", { asset: "XLM" })).resolves.toBe(1000);
    await expect(getMetricValue("payments_volume_total", { asset: "USDC" })).resolves.toBe(500);
  });
});
...
describe("metricsService — error_count_total", () => {
  it("increments by service and error_code", async () => {
    ...
    await expect(
      getMetricValue("error_count_total", { service: "horizon", error_code: "TIMEOUT" }),
    ).resolves.toBe(1);
  });
});
