...
exports.up = function (knex) {
  return knex.schema
    .createTable("notification_preferences", (table) => {
      ...
      table
        .string("public_key", 56)
        .notNullable()
        .unique()
        .comment("Stellar account G… address");
      table
        .jsonb("event_channels")
        .defaultTo("{}")
        .comment(
          "JSON map: { event_type: { push: bool, email: bool, in_app: bool } }",
        );
      table
      ...
      table
        .string("timezone", 64)
        .defaultTo("UTC")
        .comment("User timezone (IANA tz database)");
      table
        .boolean("push_enabled")
        .defaultTo(true)
        .comment("Master push channel toggle");
      table
        .boolean("email_enabled")
        .defaultTo(true)
        .comment("Master email channel toggle");
      table
        .boolean("in_app_enabled")
        .defaultTo(true)
        .comment("Master in-app channel toggle");
    ...
    .createTable("notification_history", (table) => {
      ...
      table
        .string("channel", 16)
        .notNullable()
        .comment("Delivery channel: push, email, in_app");
      table
        .boolean("read")
        .defaultTo(false)
